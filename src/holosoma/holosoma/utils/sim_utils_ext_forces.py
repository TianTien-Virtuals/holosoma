...
class DirectSimulation:
    ...
    def _apply_external_forces(self, step_count: int) -> None:
        """Apply hardcoded external forces every 5000 steps.
        ...
        """
        # Only for MuJoCo simulator
        if not hasattr(self.simulator, 'root_model') or not hasattr(self.simulator, 'root_data'):
            return
        
        # Only apply forces every 100 steps
        ...
        data = self.simulator.root_data
        
        # Hardcoded force and torque values
        FORCE = [500.0, 0.0, 0.0]   # 200N in X direction
        TORQUE = [0.0, 0.0, 0.0]    # No torque
        
        try:
            body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "robot_right_knee_link")
            print(f"body_id: {body_id}")
            data.xfrc_applied[body_id][:3] = FORCE
            data.xfrc_applied[body_id][3:] = TORQUE
            logger.info(f"Step {step_count}: Applied force {FORCE}N, torque {TORQUE}Nm to torso")
        except Exception as e:
            logger.warning(f"Failed to apply force at step {step_count}: {e}")
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                self._apply_external_forces(step_count)
